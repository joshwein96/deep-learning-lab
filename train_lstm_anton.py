...
from models_anton import get_model
...
wandb.login(key='5a7d897c04bcdd9c33968e3071f54b55fac7dfd0')
...
# configure model
model = get_model(args, x_train.size(2), device)
model.float()
model.to(device)
...
def test_model(model):
    ...
    for inputs, labels in iter(test_loader):
        ...
        for label, output in zip(labels, outputs.cpu().detach().numpy()):
            print(label.size())
            confusion[int(label[0].item()), int(np.argmax(output))] += 1
